...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        try {
            if (!chain.containsKey(first)) {
                ProbabilityDistribution<String> firstPD = new ProbabilityDistribution<>();
                firstPD.record(second);
                chain.put(first, firstPD);
            } else {
    ...
    public void train(Iterator<String> sentence) {
        try {
            ...
            if (sentence.hasNext()) {
                ...
                startWords.record(firstWord);
            }
            while (sentence.hasNext()) {
                secondWord = sentence.next();
                addBigram(firstWord, secondWord);
                firstWord = secondWord;
            }
            if (firstWord != null) {
                addBigram(firstWord, null);
            }
        } catch (NullPointerException e) {
            throw new IllegalArgumentException();
        }
    ...
    public boolean hasNext() {
        ...
        return chain.get(mcString) != null;
    }
    ...
    @Override
    public String next() {
        String temp = mcString;
        if (mcString == null) {
            throw new NoSuchElementException();
        }
        try {
            if (chain.containsKey(temp)) {
                if (chain.get(mcString) != null) {
                    mcString = chain.get(mcString).pick(ng);
                } else {
                    mcString = null;
                }
            }
