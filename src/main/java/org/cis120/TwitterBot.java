...
public class TwitterBot {
    ...
    public TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
        ...
        this.ng = ng;
        List<List<String>> tweets = new LinkedList<>();
        tweets = TweetParser.csvDataToTrainingData(br, tweetColumn);
        for (List<String> tempA : tweets) {
            Iterator<String> a = tempA.iterator();
            mc.train(a);
        }
    }
    ...
    public void writeStringsToFile(
        ...
        try {
            ...
            for (String stLine : stringsToWrite) {
                bw.write(stLine + "\n");
                bw.flush();
            }
            bw.close();
    ...
    public String generateTweet(int numWords) {
        ...
        while (counter < numWords) {
            this.mc.reset();
            if (!mc.hasNext() && isEmptyMC) {
                return tweet;
            }
            sentence = "";
            while (mc.hasNext() && counter < numWords) {
                if (isFirstWord) {
                    isFirstWord = false;
                    sentence += mc.next();
                } else {
            ...
            tweet += sentence + randomPunctuation();
        }
        return tweet;
    }
